...
if(store_plot == TRUE){
  if(lower_fatalitiy_thresh == 0){
    ggsave("plots_fatalities_greq1/roc_curve_selected_models_prev_peace_greq1.png",
           plot = roc_curve_selected_models, width = 1.2 * 3391, height = 1.2 * 1225, dpi = 300, units = "px",
           bg="white")
  } else {
    ggsave("plots_fatalities_greq25/roc_curve_selected_models_prev_peace_greq25.png",
           plot = roc_curve_selected_models, width = 1.2 * 3391, height = 1.2 * 1225, dpi = 300, units = "px",
           bg="white")
  }
}
...
if (store_plot == TRUE) {
  ...
  if (lower_fatalitiy_thresh == 0) {
    ggsave(
      "plots_fatalities_greq1/reliability_diagram_selected_models_prev_peace_1.png",
    ...
    ggsave(
      "plots_fatalities_greq25/reliability_diagram_selected_models_prev_peace_25.png",
      plot = reliability_grid,
      width = 1.0 * 3000, height = 1.0 * 1800, dpi = 300, units = "px",
      bg = "white"
    )
...
if(store_plot == TRUE){
  if(lower_fatalitiy_thresh == 0){
    ggsave("plots_fatalities_greq1/roc_reliability_selected_models_prev_peace_greq1.png",
           plot = roc_reliability_plot, width = 1.2 * 1000, height = 1.2 * 2000, dpi = 300, units = "px",
           bg="white")
  } else {
    ggsave("plots_fatalities_greq25/roc_reliability_selected_models_prev_peace_greq25.png",
           plot = roc_reliability_plot, width = 1.2 * 1000, height = 1.2 * 2000, dpi = 300, units = "px",
           bg="white")
  }
}
...
if(store_plot == TRUE){
  if(lower_fatalitiy_thresh == 0){
    ggsave("plots_fatalities_greq1/brier_score_decomposition_selected_models_prev_peace_greq1.png",
           plot = brier_score_decomposition_selected, width = 1.0 * 4222, height = 1.0 * 2313, dpi = 300, units = "px",
           bg="white")
  } else {
    ggsave("plots_fatalities_greq25/brier_score_decomposition_selected_models_prev_peace_greq25.png",
           plot = brier_score_decomposition_selected, width = 1.0 * 4222, height = 1.0 * 2313, dpi = 300, units = "px",
           bg="white")
  }
}
...
if(store_plot == TRUE){
  if(lower_fatalitiy_thresh == 0){
    ggsave("plots_fatalities_greq1/log_score_decomposition_selected_models_prev_peace_greq1.png",
           plot = log_score_decomposition_selected, width = 1.0 * 4222, height = 1.0 * 2313, dpi = 300, units = "px",
           bg="white")
  } else {
    ggsave("plots_fatalities_greq25/log_score_decomposition_selected_models_prev_peace_greq25.png",
           plot = log_score_decomposition_selected, width = 1.0 * 4222, height = 1.0 * 2313, dpi = 300, units = "px",
           bg="white")
  }
}
...
##-----
## Ranking of the selected models for different situations
##-----
library(patchwork)

crps_rankings_selected_models <- crps_month_selected_models %>%
  group_by(Model) %>%
  summarise(total_CRPS = sum(CRPS), .groups = "drop") %>%
  right_join(crps_month_selected_models, by = "Model") %>%
...
twcrps_rankings_selected_models <- twcrps_month_selected_models %>%
  group_by(Model) %>%
  summarise(total_twCRPS = sum(twCRPS), .groups = "drop") %>%
  right_join(twcrps_month_selected_models, by = "Model") %>%
...
model_rankings <- crps_rankings_selected_models %>%
  ...
  left_join(brier_per_peace_rankings_selected_models, by = "Model")
...
if(store_plot == TRUE){
  if(lower_fatalitiy_thresh == 0){
    ggsave("plots_fatalities_greq1/selected_models_ranking_plot_greq1.png",
           plot = selected_models_ranking_plot, width = 1.0 * 4222, height = 1.0 * 2000, dpi = 300, units = "px",
           bg="white")
  } else {
    ggsave("plots_fatalities_greq25/selected_models_ranking_plot_greq25.png",
           plot = selected_models_ranking_plot, width = 1.0 * 4222, height = 1.0 * 2000, dpi = 300, units = "px",
           bg="white")
  }
}
...
# --------------------------------------------------------------------------
model_1 = "zero"#"submission_muchlinski_thornhill"

prev_peace_prob_month_long_binary_actual %>%
  group_by(model) %>%
  summarise(brier = sum(brier_onset)/n()) %>%
  filter(model %in% selected_models) %>%
  arrange(brier) %>%
...
brier_decomposition_results_selected[[model_1]]


brier_month %>%
  filter(Model == model_1, Situation %in% c("onset","peace")) %>%
  summarise(brier = sum(Brier))
